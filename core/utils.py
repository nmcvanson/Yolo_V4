...
def traffic_recog(img, coords):
    classes_for_detection = { 0:'Speed limit (20km/h)',
    ...
    model = load_model('./yolov4-custom-functions/data/model/TSR.h5')
    xmin, ymin, xmax, ymax = coords
    cropped_img = img[int(ymin)-5:int(ymax)+5, int(xmin)-5:int(xmax)+5]
    data_image = []
    cropped_img = cropped_img.resize((30, 30))
    data_image.append(np.array(cropped_img))
    X_test=np.array(data_image)
    predict_x = model.predict(X_test) 
    Y_pred = np.argmax(predict_x,axis=1)
    s = [str(i) for i in result]
    a = int("".join(s))
    result = classes_for_detection[a]
    return result
...
def draw_bbox(image, bboxes, info = False, counted_classes = None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), traffic_sign_recog = False):
    ...
    for i in range(num_boxes):
        ...
        if class_name not in allowed_classes:
            ...
            if traffic_sign_recog:
                height_ratio = int(image_h / 25)
                result = traffic_recog(image, coor)
                cv2.putText(image, result, (int(coor[0]), int(coor[1]-height_ratio)), 
